...
def run_test(test_path, output_dir, verification_dir):
    ...
    if os.path.exists(urg_report_dir):
        print(
            f"Skipping '{test_path}' as it has already been simulated, "
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        sim_log_path = os.path.join(
            temp_dir, f"seed-{SEED}/rtl_sim", f"generated_test_{test_id}.{SEED}/sim.log"
        )
        cmd = (
            f"make SEED={SEED} SIMULATOR=vcs ISS=spike ITERATIONS=1 COV=1 "
            f"OUT={temp_dir}"
        )
        with open(os.path.join(test_output_dir, "sim.stdout"), "w") as stdout, open(
            os.path.join(test_output_dir, "sim.stderr"), "w"
        ) as stderr:
            try:
                print(f"- Command: '{cmd}'")
                subprocess.run(cmd.split(" "), stdout=stdout, stderr=stderr, check=True)
                shutil.move(
                    temp_urg_dir,  # Move urg report to test output directory
                    urg_report_dir,
                )
