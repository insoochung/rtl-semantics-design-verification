...
def pretrain(params):
    ...
    save_path = os.path.join(params["ckpt_dir"], params["ckpt_name"])
    ...
    for step in train_progress:
        ...
        for inner_step in range(accum_steps):
            with tf.GradientTape() as tape:
                y, y_labels = model.cdfg_reader.pretrain_forward(batch_size=batch_size)
                loss = loss_obj(y_labels, y)
                losses.append(loss)
                grads_list.append(tape.gradient(loss, train_vars))
        grads, loss = accumulate_gradients_and_loss(
            grads_list, losses, accum_steps, train_vars
        )
