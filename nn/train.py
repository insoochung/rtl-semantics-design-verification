...
def maybe_init_or_load_model(model, params):
    ...
    if os.path.exists(ckpt_path):
        ...
        assert os.path.exists(
            os.path.exists(os.path.dirname(warmstart_path))
...
def run_with_seed(params, run_fn=run_training):
    ...
    with open(os.path.join(params["ckpt_dir"], "meta.json"), "w") as f:
        f.write(
            json.dumps(
                m, indent=2, sort_keys=True, default=lambda o: "<not serializable>"
            )
        )
...
def set_model_flags(parser, set_required=False):
    ...
    parser.add_argument(
        "--ckpt_name", type=str, default="model.ckpt", help="Name of the checkpoint."
    )
    parser.add_argument(
        "--n_hidden", type=int, default=32, help="Number of hidden units."
    )
    parser.add_argument(
        "--n_labels", type=int, default=1, help="Number of labels in final task target."
    )
    parser.add_argument(
        "--n_gnn_layers", type=int, default=8, help="Number of GNN layers."
    )
    parser.add_argument(
        "--n_lstm_layers", type=int, default=1, help="Number of LSTM layers."
    )
    parser.add_argument(
        "--n_mlp_hidden",
        type=int,
    ...
    parser.add_argument(
        "--lr_scheme", type=str, default="constant", help="Learning rate scheme."
    )
    parser.add_argument(
        "--decay_rate", type=float, default=0.9, help="Learning rate decay rate."
    )
    parser.add_argument(
        "--decay_steps", type=int, default=4000, help="Learning rate decay steps."
    )
    parser.add_argument(
        "--warmup_steps", type=int, default=8000, help="Learning rate warm up steps."
    )
    parser.add_argument("--batch_size", type=int, default=32, help="Batch size.")
    parser.add_argument(
        "--epochs", type=int, default=50, help="Number of epochs to train."
    )
    parser.add_argument("--seed", type=int, default=123, help="Seed to set.")
    parser.add_argument(
        "--append_seed_to_ckpt_dir",
        action="store_true",
        default=False,
        help="Append the seed path/dir vars.",
    )
    parser.add_argument(
        "--aggregate",
        type=str,
        default="mean",
        help="How the CDFG reader will aggregate coverpoint " "embedding",
    )
    parser.add_argument(
        "--use_attention",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        default=4096,
        help="Max number of nodes that CDFG reader should " "process.",
    )
    parser.add_argument(
        "--use_custom_attention_hparams",
        action="store_true",
        default=False,
    ...
    parser.add_argument("--use_att_encoder", action="store_true", default=False)
    parser.add_argument(
        "--no_att_encoder", dest="use_att_encoder", action="store_false", default=False
    )
    parser.add_argument("--use_att_decoder", action="store_true", default=True)
    parser.add_argument(
        "--no_att_decoder", dest="use_att_decoder", action="store_false", default=True
    )
    parser.add_argument("--freeze_att_encoder", action="store_true", default=False)
    parser.add_argument("--design_feedback_type", type=str, default="cdfg_reader")
    parser.add_argument("--n_max_coverpoints", type=int, default=2000)
    parser.add_argument(
        "--cdfg_cutoff", dest="cdfg_cutoff", action="store_true", default=False
    )
