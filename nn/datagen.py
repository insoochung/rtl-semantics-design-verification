...
def split_dataset(dataset, split_ratio):
    ...
    if test:
        test_ds = tf.data.experimental.sample_from_datasets([dataset[i] for i in test])
...
def convert_to_tf_dataset(dataset, label_key="is_hits"):
    ...
    def ds_gen(dataset):
        for i in range(dataset["coverpoint"].shape[0]):
            yield (
                {k: dataset[k][i] for k in dataset.keys()},  # inputs
                dataset[label_key][i],
            )  # label
    ...
    for key in dataset:
        ...
        signature[key] = tf.TensorSpec(
            shape=dataset[key].shape[1:] if nd > 1 else (), dtype=dataset[key].dtype
        )
        if key == label_key:
...
def combine_data_per_cp(
    cp_idx, dataset, cp_idx_to_midx, cp_idx_to_mask, cp_idx_to_sent_vec
):
    tp_vectors = []
    is_hits = []
    masks = []
    ...
    print(
        f"Dataset for coverpoint '{cp_idx}' shapes: "
        f"{[f'{k}: {v.shape}' for k, v in ret.items()]}"
    )
    return ret


def combine_data(graph_dir, tp_cov_dir, hit_lower_bound=0.1, hit_upper_bound=0.9):
    print("Aggregating sporadic information to finalize dataset...")
    # Load dataset
    design_graph_filepath = os.path.join(graph_dir, "design_graph.pkl")
    assert os.path.exists(design_graph_filepath), f"'{design_graph_filepath}' not found"
    ...
    print(
        f"# of coverpoints within hit rate range between "
    ...
    for cp_idx in tqdm.tqdm(cov_dataset.keys()):
        ...
        cp_idx_to_mask[cp_idx] = bvocab.get_mask(
            cp_idx, midx, mask_length=graphs[0].n_nodes
        )
    ...
    for cp_index in cp_indices:
        dataset[cp_index] = combine_data_per_cp(
            cp_index, cov_dataset, cp_idx_to_midx, cp_idx_to_mask, cp_idx_to_sent_vec
        )
...
def get_fake_inputs(tf_data_dir, n_samples=2):
    ...
    es_info = {
        k: ([n_samples] + (list(v.shape) if v.shape != () else [1]), v.dtype)
        for k, v in es.items()
    }
...
def save_dataset(dataset, tf_data_dir):
    ...
    for k, ds in dataset.items():
        tf.data.experimental.save(
            ds,
            os.path.join(tf_data_dir, f"coverpoint.{k:05d}.tfrecord"),
            compression="GZIP",
        )
...
def generate_data(
    ...
    dataset = combine_data(graph_dir, tp_cov_dir, hit_lower_bound, hit_upper_bound)
