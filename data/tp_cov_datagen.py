...
from cdfg.constructor import DesignGraph, Module, construct_design_graph
...
def get_dataset_utilites(test_templates_dir: str, output_dir: str):
    ...
    if not vocab_files:
        ...
        vocab_filepath = os.path.join(output_dir, f"vocab.{len(vocab.tokens)}.yaml")
...
def get_cdfg_subpath(
    branch_line_nums: List[int],
    trace_conditions: Tuple[int],
    ...
    for i, cond in enumerate(trace_conditions):
        if cond is None:
            continue
        assert cond in branch_conditions[i], f"{cond} not in {branch_conditions[i]}"
    ...
    assert all([n in cdfg_subpath or not c for (n, c) in nodes_and_conditions]), (
        "Trace subpath should include all branch nodes with non-dont-care " "conditions"
    )
    ...
    if subpath_signature in covered_subpaths:
        ...
        for i, (ct, t) in enumerate(zip(covered_trace, trace_conditions)):
            if ct != t:
                assert set({ct, t}) == set({None, Condition.TRUE}) and isinstance(
                    branch_nodes[i].next_nodes[1][0], EndNode
                )
    covered_subpaths[subpath_signature] = (branch_line_nums[0], tuple(trace_conditions))
    return cdfg_subpath


def generate_dataset_inner(
    test_dir: str,
    ...
    if not os.path.isdir(sim_cov_dir):
        print(
            f"Looked for extracted coverage information in '{sim_cov_dir}' "
            f"but no luck."
        )
        return
    ...
    assert len(testfiles) == 1, f"There should be only one test file within {test_dir}"
    ...
    if irrelevant:
        ...
        print(
            "These modules may be part of the testbench but not the actual "
    ...
    for module_name in modules:
        print(
            f"Syncing coverage for module: {module_name} - parsed from "
    ...
    tp_vector_list = [np.array(elem, dtype=np.float32) for elem in tp_vector_list]
    is_hit_list = [np.array(elem, dtype=np.float32) for elem in is_hit_list]
    coverpoint_idx_list = [
...
def generate_dataset(
    simulated_tests_dir: str,
    design_graph_dir: str,
    output_dir: str,
    test_templates_dir: str = "",
):
    """Generates the dataset from the given coverage and CDFG directories."""
    test_dirs = []
    ...
    assert len(test_dirs) > 0, f"No test directories found in '{simulated_tests_dir}'"
...
def main():
    ...
    parser.add_argument(
        ...
        default=False,
        required=True,
        help=("Directory where tests and relevant coverage " "information is located."),
    )

    # CDFG related arguments
    parser.add_argument(
        "-cd",
        "--construct_design_graph",
        default=False,
        action="store_true",
    ...
    parser.add_argument(
        "-rd", "--rtl_dir", help="Directory where the original RTL files are " "located"
    )
    parser.add_argument(
        "-gd",
        "--design_graph_dir",
    ...
    parser.add_argument(
        "-tt", "--test_templates_dir", default="", help="Directory to test templates"
    )
    ...
    generate_dataset(
        simulated_tests_dir, design_graph_dir, output_dir, test_templates_dir
    )
