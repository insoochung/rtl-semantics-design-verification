...
class GraphHandler:
    ...
    def load_or_generate_vocab(self):
        ...
        for n in self.design_graph.nodes:
            ...
            block_depth_info["min"] = min(block_depth_info["min"], n.block_depth)
            block_depth_info["max"] = max(block_depth_info["max"], n.block_depth)
    ...
    def vectorize_design_graph(self):
        ...
        print(
            f"Graphs shape: {self.graphs.shape} // " f"(batch, num_nodes, num_features)"
        )
        print(
            f"Adjacency matrix shape: {self.adjs.shape} // "
...
def main():
    ...
    parser.add_argument(
        "-dg",
        "--design_graph_filepath",
        type=str,
        required=True,
        help="Path to the pickeld design graph file",
    )
    parser.add_argument(
        "-od", "--output_dir", type=str, help="Where to save the output files"
    )
