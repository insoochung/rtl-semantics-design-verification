...
class TestParameterCoverageHandler:
    ...
    def load_from_file(self):
        ...
        print(
            f"Loaded datapoints from dataset: {self.filepath} "
    ...
    def add(self, tp_vectors, is_hits, coverpoints):
        ...
        print(
            f"- current shapes: tp_vectors {self.data['tp_vectors'].shape}, "
    ...
    def arrange_dataset_by_coverpoint(self):
        ...
        for i in tqdm.tqdm(range(self.data["tp_vectors"].shape[0])):
            ...
            cov_to_dp[coverpoint]["tp_vectors"] = np.append(
                cov_to_dp[coverpoint]["tp_vectors"], tp_vector.reshape(1, -1), axis=0
            )
            cov_to_dp[coverpoint]["is_hits"] = np.append(
                cov_to_dp[coverpoint]["is_hits"], is_hit, axis=0
            )
        hit_rates = []
        num_th = 0
...
class NodeVocab:
    ...
    def add_node_info(self, element_name, element_type, element_info):
        element = {"name": element_name, "type": element_type, "info": element_info}
...
class BranchVocab:
    ...
    def get_mask(
        ...
        return_module_index: bool = False,
    ):
    ...
    def get_sentence_vector(
        self, nodes: list, branch_index: int, module_index: int = None
    ):
        if not self.s2v:
...
class TestParameterVocab:
    def __init__(self, vocab_filepath: str = "", test_templates_dir: str = ""):
        ...
        self.meta = None
        assert (
            vocab_filepath or test_templates_dir
    ...
    def generate_from_test_template(self):
        test_parameters = {}
        tests = set()
        for test_template_path in glob(os.path.join(self.test_templates_dir, "*.yaml")):
