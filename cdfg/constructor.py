...
from cdfg.graph import Node, BranchNode, EndNode, AlwaysNode, DummyNode
from cdfg.parser import get_verible_parsed_rtl
from cdfg.utils import (
    preprocess_rtl_str,
    find_subtree,
    get_subtree_text,
    get_leftmost_node,
...
def connect_nodes(node: "Node", next_node: "Node", condition=None):
    """Connect two nodes."""
...
def construct_statement(verible_tree: dict, rtl_content: str, block_depth: int = 0):
    if is_ternary_assignment(verible_tree) or is_ternary_expression(verible_tree):
        return construct_statement_with_ternary_cond(
            verible_tree, rtl_content, block_depth=block_depth
        )
    else:
        return construct_terminal(verible_tree, rtl_content, block_depth=block_depth)


def construct_terminal(verible_tree: dict, rtl_content: str, block_depth: int = 0):
...
def construct_if_else_statement(
    verible_tree: dict, rtl_content: str, block_depth: int = 0
):
    ...
    if block_node["tag"] not in Tag.IGNORE:
        assert (
            block_node["tag"] in Tag.TERMINAL_STATEMENTS + Tag.BLOCK_STATEMENTS
...
def construct_case_statement(
    ...
    branch_node = BranchNode(
        verible_tree=verible_tree, rtl_content=rtl_content, block_depth=block_depth
    )
    ...
    for case_item in get_case_item_tree(verible_tree)["children"]:
        ...
        assert (
            children_tags[1] == Tag.COLON
            or children_tags[0] == Tag.DEFAULT
            and children_tags[1] == None
        )
        ...
        if children_tags[2] in Tag.BLOCK_STATEMENTS:
            ...
            node = construct_statement(node, rtl_content, block_depth=block_depth + 1)
...
def construct_for_loop_statement(
    verible_tree: dict, rtl_content: str, block_depth: int = 0
):
    ...
    if children[1]["tag"] == Tag.SEQ_BLOCK:
        ...
        nodes = construct_block(children[1], rtl_content, block_depth=block_depth + 1)
    else:
        # When a loop body is a single statement, construct a statement only
        nodes = construct_statement(
            children[1], rtl_content, block_depth=block_depth + 1
        )
...
def is_ternary_assignment(verible_tree: dict):
    ...
    return verible_tree["tag"] in Tag.ASSIGNMENTS and find_subtree(
        verible_tree, Tag.TERNARY_EXPRESSION
    )


def is_ternary_expression(verible_tree: dict):
    """Check if the verible tree is ternary expression"""
    return verible_tree["tag"] in Tag.EXPRESSIONS and find_subtree(
        verible_tree, Tag.TERNARY_EXPRESSION
    )


def construct_statement_with_ternary_cond(
    verible_tree: dict, rtl_content: str, block_depth: int = 0
):
    """Construct ternary assignment from verible tree"""
    assert is_ternary_assignment(verible_tree) or is_ternary_expression(verible_tree)
    ...
    start_node = Node(verible_tree, rtl_content=rtl_content, block_depth=block_depth)
    branch_node = BranchNode(
        verible_tree=ternary_tree, rtl_content=rtl_content, block_depth=block_depth
    )
    trees = {
    ...
    for cond, tree in trees.items():
        nodes[cond] = construct_statement(
            tree, rtl_content, block_depth=block_depth + 1
        )

    # Construct start and end nodes.
    end_node = Node(verible_tree, rtl_content=rtl_content, block_depth=block_depth)
...
def construct_block(verible_tree: dict, rtl_content: str, block_depth: int = 0):
    ...
    if tag == Tag.SEQ_BLOCK:
        return construct_seq_block(verible_tree, rtl_content, block_depth=block_depth)
    elif tag == Tag.IF_ELSE_STATEMENT:
        return construct_if_else_statement(
            verible_tree, rtl_content, block_depth=block_depth
        )
    elif tag == Tag.CASE_STATEMENT:
        return construct_case_statement(
            verible_tree, rtl_content, block_depth=block_depth
        )
    elif tag == Tag.FOR_LOOP_STATEMENT:
        return construct_for_loop_statement(
            verible_tree, rtl_content, block_depth=block_depth
        )
    else:
...
def construct_seq_block(verible_tree: dict, rtl_content: str, block_depth: int = 0):
    ...
    for statement in statement_list["children"]:
        ...
        if tag in Tag.BRANCH_STATEMENTS + [Tag.SEQ_BLOCK]:
            new_nodes = construct_block(statement, rtl_content, block_depth=block_depth)

        elif tag == Tag.FOR_LOOP_STATEMENT:
            new_nodes = construct_block(statement, rtl_content, block_depth=block_depth)
        elif tag in Tag.TERMINAL_STATEMENTS:
            new_nodes = construct_statement(
                statement, rtl_content, block_depth=block_depth
            )
        elif tag == Tag.MACRO_GENERIC_ITEM:
            new_nodes = construct_terminal(
                statement, rtl_content, block_depth=block_depth
            )
...
def construct_always_node(verible_tree: dict, rtl_content: str, block_depth: int = 0):
    ...
    if always_node.type in ["always_ff", "always"]:
        ...
        if len(content) == 2 and content[0]["tag"] == Tag.ALWAYS_CONDITION:
            ...
            always_node.condition = get_subtree_text(condition, always_node.rtl_content)
    if not body:
        body = children[1]

    if body["tag"] in Tag.BLOCK_STATEMENTS:
        body_nodes = construct_block(
            body, always_node.rtl_content, block_depth=always_node.block_depth + 1
        )
        assert body_nodes, "Seq block is empty."
        # Arbitrary end always_node.
    else:
        body_nodes = construct_statement(
            body, always_node.rtl_content, block_depth=always_node.block_depth + 1
        )
    ...
    always_node.update_text_and_type(force_end=get_leftmost_node(body_nodes[0]).start)
...
class DesignGraph:
    ...
    def construct_modules(self):
        ...
        parsed_rtl = get_verible_parsed_rtl(self.parsed_rtl_dir, orig_dir=self.rtl_dir)
        for filepath, verible_tree in parsed_rtl.items():
            ...
            for module_subtree in find_subtree(verible_tree, Tag.MODULE):
                ...
                if len(module.always_graphs) == 0:
                    print(
                        f"-- Skipping {filename} because it has no always blocks --\n"
                    )
                    del module
                    continue
                self.modules.append(module)
        print(f"-- CDFGs successfully constructed from design {self.rtl_dir}! --\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Directory where verible generated ASTs "
