...
class Node:
    ...
    def __init__(
        self, verible_tree: dict = None, rtl_content: str = "", block_depth: int = 0
    ):
    ...
    def __str__(self):
        ...
        if self.next_nodes:
            prefix += (
                f" / fan_out: " f"{[(n[0].get_id(), n[1]) for n in self.next_nodes]}"
            )
    ...
    def update_text_and_type(self, force_start=None, force_end=None):
        ...
        text_info = get_subtree_text_info(self.verible_tree, self.rtl_content)
    ...
    def add_next_node(self, next_node: "Node", next_condition: str = None):
        ...
        if next_condition == Condition.TRUE:
            ...
            if not isinstance(next_condition, list):
                ...
                next_conditions = [f"{self.condition} == {c}" for c in next_condition]
...
class BranchNode(Node):
    ...
    def update_condition(self):
        ...
        text_info = get_subtree_text_info(condition_tree, self.rtl_content)
...
class AlwaysNode(Node):
    ...
    def update_text_and_type(self, force_start=None, force_end=None):
        ...
        assert "always" in self.type, f"{self.type} is not a inspected type of node."

    def update_condition_vars(self):
        """Find and track the vars used in descendent condition statements"""
        conditional_subtrees = find_subtree(self.verible_tree, Tag.CONDITION_STATEMENTS)
        ids = set()
